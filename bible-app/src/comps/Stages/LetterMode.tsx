...
const LetterMode = ({ oneLetterMode, changeLearnMethods }: Props) => {
  return (
    <>
      <div className="absolute bottom-20 right-5 left-5 p-2 flex justify-center items-center gap-4 text-xl font-Inter">
        <span className={`${oneLetterMode ? "text-green-500" : "text-white"} transition-colors duration-500`}>
          1st Letter
        </span>
        <div
        ...
        <span className={`${oneLetterMode ? "text-white" : "text-green-500"}  transition-colors duration-500`}>
          Full Word
        </span>
      </div>
    </>
